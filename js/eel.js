eel = {
	...
	_init: function() {
		...
		document.addEventListener( 'DOMContentLoaded', event => {
			...
			// let addr = sessionStorage.getItem( 'eel.guid' )
			// if ( ! addr ) {
			// 	addr = eel.guid()
			// 	sessionStorage.setItem( 'eel.guid', addr )
			// }
			// const websocket_addr = `${ eel._host }/eel?page=${ addr }`.replace( 'http', 'ws' )
			const websocket_addr = `${ eel._host }/eel?page=${ page }`.replace( 'http', 'ws' )
			console.log(websocket_addr)
			eel._websocket = new WebSocket( websocket_addr )

			eel._websocket.onopen = () => {
				for ( let py_function of eel._py_functions )
					eel._import_py_function( py_function )

				while ( eel._mock_queue.length > 0 )
					eel._websocket.send( eel._toJSON( eel._mock_queue.shift() ))
			}
